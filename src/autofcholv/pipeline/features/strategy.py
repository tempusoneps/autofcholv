...
REQUIRED_HELPER_COLUMNS = {
    "_temp_accept_long_4_shift1",
    "adx_14",
    "adx_42",
    "ibs",
    "bearish_low_break_candle",
    "body_atr_ratio",
    "_temp_body_rate_first_close",
    ...
    "macd_hist_12_26_9",
    "mom_y",
    "_temp_morning_breakout_long",
    "open_range_high_2",
    "open_range_low_2",
    "_temp_opening_gap_pct",
...
def extract_features(df: pd.DataFrame, _config: Config) -> pd.DataFrame:
    ...
    momentum_long = not_extreme & (df["mom_y"] > 0.26) & (df["_temp_body_rate_first_close"] > 0.65)
    momentum_short = not_extreme & (df["mom_y"] < -0.18) & (df["_temp_body_rate_first_close"] < -0.39)
    ...
    df["signal_pro2"] = _signal_from_conditions(
        df["session_0935_1425"] & in_orb & close_gt_orh & close_gt_ema55 & vol_ok & (df["_temp_opening_gap_pct"] > 0.10),
        df["session_0935_1425"] & in_orb & close_lt_orl & close_lt_ema55 & vol_ok & (df["_temp_opening_gap_pct"] < -0.10),
    )
    ...
    df["signal_pro19"] = _signal_from_conditions(channel_session & (df["donchian_high_10_shift1"]) & (df["macd_hist_12_26_9"] > 0) & close_gt_ema55 & vol_ok, channel_session & (df["donchian_low_10_shift1"]) & (df["macd_hist_12_26_9"] < 0) & close_lt_ema55 & vol_ok)
    ...
    df["signal_pro29"] = _signal_from_conditions(df["entry_window_1300_1425"] & (df["_temp_accept_long_4_shift1"] >= 0.42) & (df["_temp_morning_breakout_long"] >= 0.0) & (df["ibs"] >= 0.55) & (df["rsi_8"] >= 54) & (df["linear_regression_slope_5"] > 0), pd.Series(False, index=df.index))

    for signal_index in range(1, 30):
        column = f"signal_pro{signal_index}"
        df[column] = df[column].fillna(SIGNAL_NONE)
        
    df["_temp_signal_pro4"] = df["signal_pro4"]
    df = df.drop(columns=["signal_pro4"], errors="ignore")
    return df
